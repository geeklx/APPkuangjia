package com.haier.cellarette.baselibrary.recycleviewgallery.discretescrollviews.util;
...
public class DiscreteScrollUtil {

    private static DiscreteScrollUtil instance;

    private final String KEY_TRANSITION_TIME;

    public static void init(Context context, String sp_default_Prefs) {
        instance = new DiscreteScrollUtil(context, sp_default_Prefs);
    }

    private DiscreteScrollUtil(Context context, String sp_default_Prefs) {
        KEY_TRANSITION_TIME = sp_default_Prefs;
    }
    ...
    private static SharedPreferences defaultPrefs() {
        return PreferenceManager.getDefaultSharedPreferences(BaseApp.get());
    }

    public static void configureTransitionTime1(DiscreteScrollView scrollView) {
        ...
        bsd.setContentView(R.layout.activity_recycleviewgallery_dialog_transition_time1);
    ...
    public static void configureTransitionTime2(DiscreteScrollView scrollView) {
        final BottomSheetDialog bsd = new BottomSheetDialog(scrollView.getContext());
        final TransitionTimeChangeListener timeChangeListener = new TransitionTimeChangeListener(scrollView);
        bsd.setContentView(R.layout.activity_recycleviewgallery_dialog_transition_time2);
        defaultPrefs().registerOnSharedPreferenceChangeListener(timeChangeListener);
        bsd.setOnDismissListener(new DialogInterface.OnDismissListener() {
            @Override
            public void onDismiss(DialogInterface dialog) {
